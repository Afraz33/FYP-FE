...
function ForgotPassword() {
  ...
  const handleForgotPassword = async () => {
    try {
      const response = await axios.post('http://localhost:5000/api/focalperson/forgot-password', { universityEmail });
      setMessage(response.data.message);
    } catch (error) {
      console.error('Error during forgot password:', error);
      if (error.response && error.response.status === 404) {
        setMessage('User not found. Please make sure you have entered the correct email address.');
      } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen font-Onest bg-gray-100">
      <div className="w-full max-w-2xl p-8 bg-white rounded shadow-md">
        ...
        <p className="mb-4 text-center">Enter your university email to reset your password</p>
        <input
          type="email"
