    ...
    if (!universityEmail || !password) {
      ...
      try {
        const response = await axios.post(
          "http://localhost:5000/api/focalperson/login-focalperson",
        ...
        if (response.status === 200 && response.data.token) {
          ...
          setError("");
          navigate("/focalPersonLandingPage");
        } else {
  ...
  return (
    <div className="bg-gray-200">
      ...
      <div className="mt-6 w-[70%] mx-auto flex flex-col items-center font-Onest">
        ...
        >
          <Link to="/focalperson-signup" className="w-full h-full inline-block">
            Create Account
          </Link>
        </button>
      </div>
...
export default FocalPersonLogin;
